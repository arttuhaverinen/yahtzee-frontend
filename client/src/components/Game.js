...
const Game = ({ config }) => {
  ...
  return (
    <div className="pageGrid container">
      ...
      {showLocalTips && (
        <div className="div-local-advice">
          <div>
            <h4>Tips:</h4> <button onClick={() => setShowLocalTips(!showLocalTips)}>X</button>
          </div>
          <li>Hover over the dices to change their background.</li>
          <li>When you press "ROLL", you will reroll grayed out dices and keep the rest.</li>
        </div>
      )}
    </div>
  );
